...
- 2026-05-21 final Live page correction: the Live Controls launch hub should use the compact old card style, keep the copied-strategy mismatch subtext removed, and add only one extra Risk Config dropdown. Live strategy source and live risk/account portfolio settings remain separate internally; the copied Live Strategy slot remains the signal source, and the selected Risk Config controls account size, sizing, exposure, commission, slippage, and profit target for live order sizing.
...
- 2026-05-21: Superseded follow-up pass: briefly replaced the old launch-card cluster with a full Live Risk Portfolio Config builder. User rejected the full builder and asked for the compact old style with only one additional settings dropdown.
- 2026-05-21: User corrected the UI target again: do not show the full backtest-style editable risk builder. The Live page should return to the compact old launch-card style and only add one extra risk/settings config dropdown. Current frontend work restores the compact row with Topstep Account, Risk Config, Live Strategy, Account ID, and Symbols; the hidden risk values still come from the selected config dropdown and are sent on start.
- 2026-05-21: Compact Live config correction verified: `npm run build` and `./mvnw -q -DskipTests compile` passed. Browser smoke check confirmed no `Live Risk Portfolio Config` builder panel, no editable risk fields, and the Live Controls panel now shows Topstep Account = 150K Combine plus Risk Config = Topstep 150K Research alongside Live Strategy, Account ID, and Symbols.
