...
public class Orb2CurrentPolicyPerContractRunner {
	...
	public static void main(String[] args) throws Exception {
		...
		Path analysisDb = outputDir.resolve("orb2-current-policy-per-contract-" + RUN_TAG + ".db");
	...
	private static void applyOrb2OnlyCurrentPolicy() throws Exception {
		try (Connection conn = DatabaseManager.getConnection();
			 Statement stmt = conn.createStatement()) {
			stmt.executeUpdate("UPDATE FuturesStrategySettings SET settingValue='false' WHERE settingKey LIKE '" + SLOT + ".%.enabled'");
			stmt.executeUpdate("UPDATE FuturesStrategySettings SET settingValue='true' WHERE settingKey LIKE '" + SLOT + ".%.orb.enabled'");
			stmt.executeUpdate("UPDATE FuturesStrategySettings SET settingValue='false' WHERE settingKey LIKE '" + SLOT + ".%.allowOrbLongs'");
			stmt.executeUpdate("UPDATE FuturesStrategySettings SET settingValue='false' WHERE settingKey LIKE '" + SLOT + ".%.allowOrbShorts'");
			stmt.executeUpdate("UPDATE FuturesStrategySettings SET settingValue='3' WHERE settingKey LIKE '" + SLOT + ".%.orb.maxTradesPerDay'");
		}
	}
