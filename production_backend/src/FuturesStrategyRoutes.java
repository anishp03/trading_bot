package com.tradingbot.api.routes;

import com.tradingbot.ApiRequestUtils;
import com.tradingbot.FuturesManager;

import io.javalin.Javalin;
import io.javalin.http.Context;

public final class FuturesStrategyRoutes {
    ...
    private static void applyStrategySettings(Context ctx, FuturesManager.FuturesStrategySettings settings) {
        ...
        settings.orbEventPack.enabled = ApiRequestUtils.parseBooleanOrDefault(ctx.queryParam("orbEventPackEnabled"), settings.orbEventPack.enabled);
        settings.orbEventPack.maxTradesPerDay = ApiRequestUtils.parseIntOrDefault(ctx.queryParam("orbEventPackMaxTradesPerDay"), settings.orbEventPack.maxTradesPerDay);
        settings.volumePriceManipulation.enabled = ApiRequestUtils.parseBooleanOrDefault(ctx.queryParam("volumePriceManipulationEnabled"), settings.volumePriceManipulation.enabled);
        settings.volumePriceManipulation.maxTradesPerDay = ApiRequestUtils.parseIntOrDefault(ctx.queryParam("volumePriceManipulationMaxTradesPerDay"), settings.volumePriceManipulation.maxTradesPerDay);
        ...
        settings.orbEventPackSetups = ApiRequestUtils.valueOrDefault(ctx.queryParam("orbEventPackSetups"), settings.orbEventPackSetups);
        settings.orbEventPackMaxContracts = ApiRequestUtils.parseIntOrDefault(ctx.queryParam("orbEventPackMaxContracts"), settings.orbEventPackMaxContracts);
        settings.orbEventPackRequireSimilarOrb = ApiRequestUtils.parseBooleanOrDefault(ctx.queryParam("orbEventPackRequireSimilarOrb"), settings.orbEventPackRequireSimilarOrb);
        settings.orbEventPackSimilarWindowBars = ApiRequestUtils.parseIntOrDefault(ctx.queryParam("orbEventPackSimilarWindowBars"), settings.orbEventPackSimilarWindowBars);
        settings.orbEventPackGroupOrbFamilyDailyLimit = ApiRequestUtils.parseBooleanOrDefault(ctx.queryParam("orbEventPackGroupOrbFamilyDailyLimit"), settings.orbEventPackGroupOrbFamilyDailyLimit);
        settings.orbEventPackMinBreakVolumeRatio = ApiRequestUtils.parseDoubleOrDefault(ctx.queryParam("orbEventPackMinBreakVolumeRatio"), settings.orbEventPackMinBreakVolumeRatio);
        settings.orbEventPackMinRetestVolumeRatio = ApiRequestUtils.parseDoubleOrDefault(ctx.queryParam("orbEventPackMinRetestVolumeRatio"), settings.orbEventPackMinRetestVolumeRatio);
        settings.orbEventPackMinBreakBodyPct = ApiRequestUtils.parseDoubleOrDefault(ctx.queryParam("orbEventPackMinBreakBodyPct"), settings.orbEventPackMinBreakBodyPct);
        settings.orbEventPackMinRetestBodyPct = ApiRequestUtils.parseDoubleOrDefault(ctx.queryParam("orbEventPackMinRetestBodyPct"), settings.orbEventPackMinRetestBodyPct);
        settings.orbEventPackMaxExtensionPctOfRange = ApiRequestUtils.parseDoubleOrDefault(ctx.queryParam("orbEventPackMaxExtensionPctOfRange"), settings.orbEventPackMaxExtensionPctOfRange);
        settings.orbEventPackMinRetestBarsAfterBreak = ApiRequestUtils.parseIntOrDefault(ctx.queryParam("orbEventPackMinRetestBarsAfterBreak"), settings.orbEventPackMinRetestBarsAfterBreak);
        settings.orbEventPackMaxRiskTicks = ApiRequestUtils.parseDoubleOrDefault(ctx.queryParam("orbEventPackMaxRiskTicks"), settings.orbEventPackMaxRiskTicks);
        settings.orbEventPackMaxHoldBars = ApiRequestUtils.parseIntOrDefault(ctx.queryParam("orbEventPackMaxHoldBars"), settings.orbEventPackMaxHoldBars);
        settings.orbEventPackRequireEmaAlignment = ApiRequestUtils.parseBooleanOrDefault(ctx.queryParam("orbEventPackRequireEmaAlignment"), settings.orbEventPackRequireEmaAlignment);
        settings.volumePriceMode = ApiRequestUtils.valueOrDefault(ctx.queryParam("volumePriceMode"), settings.volumePriceMode);
        settings.volumePriceStartMinute = ApiRequestUtils.parseIntOrDefault(ctx.queryParam("volumePriceStartMinute"), settings.volumePriceStartMinute);
        settings.volumePriceEndMinute = ApiRequestUtils.parseIntOrDefault(ctx.queryParam("volumePriceEndMinute"), settings.volumePriceEndMinute);
        settings.volumePriceLookbackBars = ApiRequestUtils.parseIntOrDefault(ctx.queryParam("volumePriceLookbackBars"), settings.volumePriceLookbackBars);
        settings.volumePriceMinVolumeRatio = ApiRequestUtils.parseDoubleOrDefault(ctx.queryParam("volumePriceMinVolumeRatio"), settings.volumePriceMinVolumeRatio);
        settings.volumePriceQuietVolumeRatio = ApiRequestUtils.parseDoubleOrDefault(ctx.queryParam("volumePriceQuietVolumeRatio"), settings.volumePriceQuietVolumeRatio);
        settings.volumePriceMinBodyPct = ApiRequestUtils.parseDoubleOrDefault(ctx.queryParam("volumePriceMinBodyPct"), settings.volumePriceMinBodyPct);
        settings.volumePriceMaxRiskTicks = ApiRequestUtils.parseDoubleOrDefault(ctx.queryParam("volumePriceMaxRiskTicks"), settings.volumePriceMaxRiskTicks);
        settings.volumePriceRewardRisk = ApiRequestUtils.parseDoubleOrDefault(ctx.queryParam("volumePriceRewardRisk"), settings.volumePriceRewardRisk);
        settings.volumePriceMaxHoldBars = ApiRequestUtils.parseIntOrDefault(ctx.queryParam("volumePriceMaxHoldBars"), settings.volumePriceMaxHoldBars);
        settings.volumePriceRequireHigherTimeframe = ApiRequestUtils.parseBooleanOrDefault(ctx.queryParam("volumePriceRequireHigherTimeframe"), settings.volumePriceRequireHigherTimeframe);
        settings.volumePriceMaxContracts = ApiRequestUtils.parseIntOrDefault(ctx.queryParam("volumePriceMaxContracts"), settings.volumePriceMaxContracts);
