...
public class LiveRuntimeState {
	...
	public static boolean recordRealtimeEvent(
		String hub,
		String eventType,
		String accountId,
		String contractId,
		...
		try {
			if ("market".equalsIgnoreCase(cleanOrDefault(hub, ""))) {
				return recordMarketEvent(eventType, contractId, symbol, payloadJson, receivedAt);
	...
	public static String getLiveMarksJson(String symbols, String timeframe, String accountId) {
		...
		ProjectXRealtimeManager.MarketFeedState marketFeed = ProjectXRealtimeManager.currentMarketFeedState();
		String lastEventAt = marketFeed.lastMarketEventAt;
		long feedStaleSeconds = marketFeed.lastMarketEventReceivedAtMillis > 0L
			? Math.max(0L, (System.currentTimeMillis() - marketFeed.lastMarketEventReceivedAtMillis) / 1000L)
			: -1L;
		boolean realtimeRunning = marketFeed.running;
		boolean feedFresh = realtimeRunning
			&& marketFeed.marketHubConnected
			&& marketFeed.marketGeneration > 0L
			&& feedStaleSeconds >= 0L
			&& feedStaleSeconds <= MARKET_DATA_FRESH_SECONDS;
	...
	private static boolean recordMarketEvent(String eventType, String contractId, String symbol, String payloadJson, String receivedAt) {
		...
		if ("GatewayDepth".equalsIgnoreCase(normalizedType)) {
			if (updateOrderFlowDepth(resolvedSymbol, payloadJson, eventTime)) {
	...
	private static boolean updateOrderFlowDepth(String symbol, String payloadJson, String eventTime) {
		...
		if (!depthEntries.isEmpty()) {
			...
			for (int index = 0; index < depthEntries.size(); index++) {
				applied = updateSingleOrderFlowDepth(symbol, depthEntries.get(index), eventTime) || applied;
			}
			return applied;
		}
		return updateSingleOrderFlowDepth(symbol, payloadJson, eventTime);
	}

	private static boolean updateSingleOrderFlowDepth(String symbol, String payloadJson, String eventTime) {
		...
		if (type != 6 && !isBidDepthType(type) && !isAskDepthType(type)) {
			return false;
		}
