...
public class LiqrecResearchRunner {
	...
	private static void requireResearchDatabase() {
		...
		if (dbPath == null || !dbPath.contains("/target/")) {
			throw new IllegalStateException("LiqrecResearchRunner mutates strategy settings; run it only against an explicitly selected disposable DB with -Dtradingbot.db.path=...");
		}
	}
	...
	private static void importSourceRunIfAvailable() throws Exception {
		try (Connection conn = DatabaseManager.getConnection()) {
			...
			String sourcePath = System.getProperty(SOURCE_DB_PROPERTY, "").trim();
			if (sourcePath.isEmpty()) {
				throw new IllegalStateException("Set -D" + SOURCE_DB_PROPERTY + " to an explicit copied source database.");
			}
